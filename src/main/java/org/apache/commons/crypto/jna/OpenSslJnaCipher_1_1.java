 ...
 */
class OpenSslJnaCipher_1_1 implements CryptoCipher {
     ...
     */
    public OpenSslJnaCipher_1_1(Properties props, String transformation) // NOPMD
        ...
        context = OpenSslNativeJna_1_1.EVP_CIPHER_CTX_new();
    ...
    public void init(int mode, Key key, AlgorithmParameterSpec params)
        ...
        int cipherMode = OpenSslNativeJna_1_1.OOSL_JNA_DECRYPT_MODE;
        if (mode == Cipher.ENCRYPT_MODE) {
            cipherMode = OpenSslNativeJna_1_1.OOSL_JNA_ENCRYPT_MODE;
        }
        byte[] iv;
       ...
       if(algMode == AlgorithmMode.AES_CBC) {
            switch (key.getEncoded().length) {
                case 16: algo = OpenSslNativeJna_1_1.EVP_aes_128_cbc(); break;
                case 24: algo = OpenSslNativeJna_1_1.EVP_aes_192_cbc(); break;
                case 32: algo = OpenSslNativeJna_1_1.EVP_aes_256_cbc(); break;
            default:
            ...
            switch (key.getEncoded().length) {
                case 16: algo = OpenSslNativeJna_1_1.EVP_aes_128_ctr(); break;
                case 24: algo = OpenSslNativeJna_1_1.EVP_aes_192_ctr(); break;
                case 32: algo = OpenSslNativeJna_1_1.EVP_aes_256_ctr(); break;
            default:
                throw new InvalidKeyException("keysize unsupported (" + key.getEncoded().length + ")");
            }
        }
        
        int retVal = OpenSslNativeJna_1_1.EVP_CipherInit_ex(context, algo, null, key.getEncoded(), iv, cipherMode);
        throwOnError(retVal);
        OpenSslNativeJna_1_1.EVP_CIPHER_CTX_set_padding(context, padding);
    }
    ...
    public int update(ByteBuffer inBuffer, ByteBuffer outBuffer)
        ...
        int retVal = OpenSslNativeJna_1_1.EVP_CipherUpdate(context, outBuffer, outlen, inBuffer, inBuffer.remaining());
        throwOnError(retVal);
        int len = outlen[0];
    ...
    public int doFinal(ByteBuffer inBuffer, ByteBuffer outBuffer)
        ...
        int[] outlen = new int[1];
        int retVal = OpenSslNativeJna_1_1.EVP_CipherFinal_ex(context, outBuffer, outlen);
        throwOnError(retVal);
    ...
    public void close() {
        if (context != null) {
            OpenSslNativeJna_1_1.EVP_CIPHER_CTX_reset(context);
...
//            OpenSslNativeJna_1_1.EVP_CIPHER_CTX_free(context);
        }
    }
